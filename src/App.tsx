...
export default function App() {
  ...
  const coreProcesses = snapshot?.processes.filter((process) => process.kind === "core_controller") ?? [];
  const activeCore = coreProcesses.find((process) => ["starting", "running", "stopping"].includes(process.status));
  ...
  return (
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        {tab === "assets" && <AssetsPage workspace={workspace} processes={snapshot.processes} selected={selectedAsset} busy={busy} onSelect={setSelectedAssetId} onSave={upsertAsset} onDelete={(id) => void run(() => api.deleteAsset(id), "アセットを削除しました。")} onStart={(id) => void run(() => api.startAsset(id), "アセットを起動しました。")} onStop={(id) => void run(() => api.stopAsset(id), "アセットを停止しました。")} onCreate={() => { const asset = newAsset(); setSelectedAssetId(asset.id); void upsertAsset(asset); }} />}
...
function Dashboard({ snapshot, activeCore, onStartCore, onStopCore, onLifecycle, onAssetClick, onConnectionClick }: { snapshot: WorkspaceSnapshot; activeCore: string; onStartCore: () => void; onStopCore: () => void; onLifecycle: (command: "start" | "stop" | "reset") => void; onAssetClick: (id: string) => void; onConnectionClick: (id: string) => void }) {
  ...
  return <section className="page-grid">
    ...
    <div className="panel"><div className="panel-heading"><div><p className="eyebrow">ASSETS</p><h3>アセットの実行状況</h3></div><span className="muted">クリックして編集</span></div><div className="list">{snapshot.workspace.assets.length === 0 ? <Empty label="まだアセットがありません。設定を取り込むか、アセット画面から追加してください。" /> : snapshot.workspace.assets.map((asset) => { const process = snapshot.processes.find((candidate) => candidate.owner_id === asset.id && ["running", "starting", "stopping"].includes(candidate.status)); return <button className="list-item interactive" key={asset.id} onClick={() => onAssetClick(asset.id)}><span className="dot" data-state={process?.status ?? "unknown"} /><span className="item-main"><strong>{asset.name}</strong><small>{asset.role} · {asset.activation_timing}</small></span><StatusBadge value={process?.status ?? "unknown"} /></button>; })}</div></div>
...
function AssetsPage({ workspace, processes, selected, busy, onSelect, onSave, onDelete, onStart, onStop, onCreate }: { workspace: Workspace; processes: WorkspaceSnapshot["processes"]; selected: AssetDefinition | null; busy: boolean; onSelect: (id: string) => void; onSave: (asset: AssetDefinition) => void; onDelete: (id: string) => void; onStart: (id: string) => void; onStop: (id: string) => void; onCreate: () => void }) {
  return <section className="split-page"><div className="panel asset-list-panel"><div className="panel-heading"><div><p className="eyebrow">ASSET CATALOG</p><h2>個別アセット</h2></div><button className="button primary small" disabled={busy} onClick={onCreate}>追加</button></div><p className="muted">コマンド、引数、作業ディレクトリ、環境変数、起動順序を明示的に管理します。</p><div className="asset-cards">{workspace.assets.map((asset) => { const process = processes.find((item) => item.owner_id === asset.id && ["running", "starting", "stopping"].includes(item.status)); return <button key={asset.id} className={selected?.id === asset.id ? "asset-card selected" : "asset-card"} onClick={() => onSelect(asset.id)}><span className="dot" data-state={process?.status ?? "unknown"} /><strong>{asset.name}</strong><small>{asset.role} · {asset.command.target === "native" ? "ローカル" : "WSL"}</small></button>; })}{workspace.assets.length === 0 && <Empty label="アセットがありません。" />}</div></div><div className="panel editor-panel">{selected ? <AssetEditor key={selected.id} asset={selected} allAssets={workspace.assets} busy={busy} onSave={onSave} onDelete={() => onDelete(selected.id)} onStart={() => onStart(selected.id)} onStop={() => onStop(selected.id)} /> : <Empty label="左側からアセットを選択してください。" />}</div></section>;
}

function AssetEditor({ asset, allAssets, busy, onSave, onDelete, onStart, onStop }: { asset: AssetDefinition; allAssets: AssetDefinition[]; busy: boolean; onSave: (asset: AssetDefinition) => void; onDelete: () => void; onStart: () => void; onStop: () => void }) {
  ...
  return <><div className="panel-heading"><div><p className="eyebrow">ASSET EDITOR</p><h2>{draft.name}</h2></div><div className="button-row"><button className="button subtle" disabled={busy} onClick={onStart}>起動</button><button className="button subtle" disabled={busy} onClick={onStop}>停止</button></div></div><div className="form-grid"><Field label="表示名"><input value={draft.name} onChange={(event) => patch("name", event.target.value)} /></Field><Field label="役割"><select value={draft.role} onChange={(event) => patch("role", event.target.value as AssetDefinition["role"])}>{["simulator","controller","visualizer","bridge","service","external_client","monitor","other"].map((role) => <option key={role}>{role}</option>)}</select></Field><Field label="実行ファイル / コマンド" wide><input value={draft.command.program} placeholder="例: python3.12" onChange={(event) => patchCommand("program", event.target.value)} /><small>シェル文字列ではなく実行ファイルを指定し、引数は下欄で分離します。</small></Field><Field label="引数（1行に1個）" wide><textarea value={draft.command.args.join("\n")} onChange={(event) => patchCommand("args", event.target.value.split("\n").map((value) => value.trim()).filter(Boolean))} /></Field><Field label="作業ディレクトリ" wide><input value={draft.command.cwd ?? ""} onChange={(event) => patchCommand("cwd", event.target.value || null)} /></Field><Field label="実行環境"><select value={draft.command.target === "native" ? "native" : "wsl"} onChange={(event) => patchCommand("target", event.target.value === "native" ? "native" : { wsl: { distribution: "Ubuntu" } })}><option value="native">ローカルOS</option><option value="wsl">WSL2</option></select></Field>{draft.command.target !== "native" && <Field label="WSLディストリビューション"><input value={draft.command.target.wsl.distribution} onChange={(event) => patchCommand("target", { wsl: { distribution: event.target.value } })} /></Field>}<Field label="起動タイミング"><select value={draft.activation_timing} onChange={(event) => patch("activation_timing", event.target.value as AssetDefinition["activation_timing"])}><option value="before_start">before_start</option><option value="manual">manual</option><option value="after_start">after_start</option></select></Field><Field label="依存先"><select multiple value={draft.depends_on} onChange={(event) => patch("depends_on", Array.from(event.target.selectedOptions).map((option) => option.value))}><option value="core">core</option>{allAssets.filter((candidate) => candidate.id !== draft.id).map((candidate) => <option key={candidate.id} value={candidate.id}>{candidate.name}</option>)}</select><small>複数選択できます。</small></Field><Field label="環境変数（KEY=VALUE、1行に1個）" wide><textarea value={Object.entries(draft.command.env).map(([key, value]) => `${key}=${value}`).join("\n")} onChange={(event) => patchCommand("env", parseEnv(event.target.value))} /></Field><label className="toggle"><input type="checkbox" checked={draft.enabled} onChange={(event) => patch("enabled", event.target.checked)} />起動対象に含める</label></div><div className="form-actions"><button className="button danger" disabled={busy} onClick={onDelete}>削除</button><button className="button primary" disabled={busy} onClick={() => onSave(draft)}>保存</button></div></>;
}

function TopologyPage({ snapshot, selected, onSelect, onHeartbeat }: { snapshot: WorkspaceSnapshot; selected: ConnectionSnapshot | null; onSelect: (id: string) => void; onHeartbeat: (id: string) => void }) {
  return <section className="topology-page"><div className="panel topology-canvas"><div className="panel-heading"><div><p className="eyebrow">RUNTIME TOPOLOGY</p><h2>接続と実通信</h2></div><div className="legend"><StatusBadge value="connected" /><StatusBadge value="idle" /><StatusBadge value="disconnected" /><StatusBadge value="unknown" /></div></div><p className="muted">線は設定から見つかったPDU／Endpoint／Bridge経路、色は直近観測に基づく状態です。</p><div className="topology-graph">{snapshot.connections.length === 0 ? <Empty label="接続構成を取り込むと、ここにトポロジーが表示されます。" /> : snapshot.connections.map((connection) => <button key={connection.definition.id} className={selected?.definition.id === connection.definition.id ? `edge-card ${connection.state} selected` : `edge-card ${connection.state}`} onClick={() => onSelect(connection.definition.id)}><span className="node">{connection.definition.source}</span><span className="edge"><i /><em>{connection.definition.transport}</em><i /></span><span className="node">{connection.definition.destination}</span><small>{connection.definition.pdu_names.join(", ") || "PDU未特定"}</small></button>)}</div></div><div className="panel connection-detail">{selected ? <><div className="panel-heading"><div><p className="eyebrow">CONNECTION DETAIL</p><h2>{selected.definition.label}</h2></div><StatusBadge value={selected.state} /></div><div className="detail-metrics"><Metric label="送信" value={selected.messages_sent} hint={`${selected.bytes_sent} bytes`} /><Metric label="受信" value={selected.messages_received} hint={`${selected.bytes_received} bytes`} /><Metric label="最終活動" value={selected.last_activity_at ? fmt.format(new Date(selected.last_activity_at)) : "未観測"} hint={selected.observation_source} /><Metric label="PDU" value={selected.definition.pdu_names.length || "—"} hint={selected.definition.pdu_names.join(", ") || "構成から未特定"} /></div><div className="details-table"><h3>構成と観測情報</h3>{Object.entries(selected.definition.details).map(([key, value]) => <div key={key}><span>{key}</span><code>{value}</code></div>)}{selected.latest_error && <div className="failure"><span>latest_error</span><code>{selected.latest_error}</code></div>}</div><div className="button-row"><button className="button subtle" onClick={() => onHeartbeat(selected.definition.id)}>確認イベントを記録</button></div><EventTable events={snapshot.recent_events.filter((event) => event.connection_id === selected.definition.id)} /></> : <Empty label="経路を選択すると、送受信量、最終活動、PDU、エラー、イベントを表示します。" />}</div></section>;
}
